...
// Definimos el tipo para los testimonios - ACTUALIZADO
type Testimonio = {
  id: number;
  name: string;
  location: string;
  text: string;
  image: string;
  titleSuffix: string; // Agregado titleSuffix individual
};

type TestimoniosTranslatedProps = {
  id?: string;
  translations: {
    titleSuffixes?: string[]; // Opcional para retrocompatibilidad
    items: Testimonio[];
  };
};
  ...
  return (
    <section id={id} className="w-full py-10 sm:py-16 md:py-24 bg-white">
      <div className="w-full max-w-3xl mx-auto px-4 flex flex-col items-center text-center relative transition-opacity duration-500">
        ...
        {/* Título principal del testimonio - ACTUALIZADO */}
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-4 sm:mb-6">
          <span className="text-brand-orange">{testimonioActual.name}</span>{" "}
          {testimonioActual.titleSuffix}
        </h2>

        {/* Texto del testimonio - ACTUALIZADO */}
        <p className="text-base sm:text-lg text-gray-700 mb-6 sm:mb-10">
          {testimonioActual.text}
        </p>

        {/* Información de la persona - ACTUALIZADO */}
        <div className="flex flex-col items-center mb-8 sm:mb-12">
          <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full overflow-hidden mb-3 sm:mb-4 border-2 border-brand-orange">
            <Image
              src={testimonioActual.image}
              alt={testimonioActual.name}
              width={64}
              height={64}
              className="object-cover w-full h-full"
            />
          </div>
          <h3 className="font-medium text-gray-900">{testimonioActual.name}</h3>
          <p className="text-gray-600 text-xs sm:text-sm">
            {testimonioActual.location}
          </p>
        </div>
        ...
        <div className="flex justify-center gap-2">
          {testimonios.map((testimonio, index) => (
            <button
              ...
              aria-label={`Ver testimonio de ${testimonio.name}`}
            />
          ))}
        </div>
      </div>
